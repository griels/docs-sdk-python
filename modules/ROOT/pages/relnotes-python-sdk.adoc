...
You may install previous versions using pip (after installing the required
http://developer.couchbase.com/documentation/server/4.5/sdk/python/start-using-sdk.html[dependencies])
...
Please note that, due to the end of support for Microsoft Visual Studio 2008 (VC9), which is unfortunately required to build
binary extensions for Python 2.7 on Windows, we have made the difficult decision to discontinue builds and support for the latter.

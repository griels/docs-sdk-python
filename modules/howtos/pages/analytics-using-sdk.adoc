...
try Couchbase Analytics using the Python SDK.
Intentionally, the API for analytics is very similar to that of the query service. 
In Python SDK 2.6 and 2.7, Analytics was only available on the `Bucket` object;
in Python SDK 3.0, Analytics queries are submitted using the Cluster reference, not a Bucket or Collection:
...
Common errors are listed in our xref:ref:errors-and-exceptions.adoc#analytics-errors[Errors Reference doc], with exceptions caused by resource unavailability (such as timeouts and _Operation cannot be performed during rebalance_ messages) leading to an xref:howtos:handling-errors.adoc#retry[automatic retry] by the SDK.
...
A handle is created, allowing the in-progress query to be checked, and the result returned later, without locking up resources orPythonwork bandwidth.

WARNING: In Couchbase Data Platform 6.5, Deferred Queries is in _Developer Preview_. 
In Python SDK 3.0, Deferred Queries have an Interface Level of xref:project-docs:compatibility.adoc#interface-stability[Volatile].
...
NOTE: In Python SDK 3.0, KV Ingest has an Interface Level of xref:project-docs:compatibility.adoc#interface-stability[Uncommited].
